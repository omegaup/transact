...
static ssize_t transact_switch_locked(struct transact_proc *p)
{
	struct transact_ctx *ctx = p->ctx;
	ssize_t res = 0;
		...
		unsigned long ino)
{
	...
	list_for_each(ctx_node, &dev->ctx) {
		ctx = container_of(ctx_node, struct transact_ctx, contexts);
		if (ctx->ino == ino) {
...
int transact_open(struct inode *inode, struct file *filp)
{
	...
	// Passing in the write flag means that this process is Main and should pass
	// through unobstructed. The other process should take the switch to block
	// until Main has reached its first switch point, unless it already has, so
	// it's the turn of the child.
	if ((filp->f_mode & FMODE_WRITE) == 0 && ctx->current_child != current_child) {
		transact_switch_locked(p);  // releases ctx->lock.
	} else {
		spin_unlock_irq(&ctx->lock);
	}
